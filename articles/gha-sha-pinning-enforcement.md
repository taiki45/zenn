...
一応内製のCIツールを変更して対処できなくはないのですが、内部のactionの更新はすぐに伝播されてほしいので、同一organization内のcomposite actionsが対象外になるまで待とうと思っています。機能の有効化まではできなかったですが、相対的にリスクのあるサードパーティActionについては今回の調査・対応で全てpinningできたのが収穫でした。
